...
def lbfgs(f, x0, grad_f=None, m=10, max_iter=100, tol=1e-5, device='cpu'):
    x = x0.detach().clone().to(device).requires_grad_(grad_f is None)
    ...
    def line_search_wolfe(f, xk, pk, gk, grad_f=None, alpha0=1.0, c1=1e-4, c2=0.9, max_iter=20):
        ...
        def phi_grad(alpha):
            x_new = xk + alpha * pk
            if grad_f:
                return torch.dot(grad_f(x_new), pk)
            else:
                fx = f(x_new)
                if x_new.grad is not None:
                    x_new.grad.zero_()
                fx.backward()
                return torch.dot(x_new.grad, pk)

        alpha = alpha0
        phi0 = phi(0.0)
        phi_grad0 = torch.dot(gk, pk)
    ...
    while torch.norm(g) > tol and k < max_iter:
        q = g.clone()
        alpha_list = []
        ...
        for i in range(len(s_list)):
            ...
            r += s * (alpha_list[len(s_list) - 1 - i] - beta)
        ...
        if torch.dot(y, s) > 1e-10:
            if len(s_list) == m:
                s_list.pop(0), y_list.pop(0), rho_list.pop(0)
            s_list.append(s)
            y_list.append(y)
            rho_list.append(1.0 / torch.dot(y, s))

        x = x_new.detach().clone().requires_grad_(grad_f is None)
        g = g_new
        k += 1

    return x.detach(), f(x).item(), k
