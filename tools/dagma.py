...
class DagmaLinear:
    ...
    def _score(self, W: np.ndarray) -> typing.Tuple[float, np.ndarray]:
        ...
        if self.loss_type == 'l2':
            ...
            QC = self.Qinv @ self.C
            QWPhi = self.Qinv @ W @ self.Phi.T + self.Qinv @ W @ self.Phi

            loss = 0.5 * self.K * np.trace(QSigma - QCW - QWC + QWPhiW)
            G_loss = 0.5 * self.K * (-2*QC + QWPhi)
        return loss, G_loss
    ...
    def minimize(self, 
        ...
        for iter in range(1, max_iter+1):
            ...
            if self.loss_type == 'l2':
                ...
                G_score = mu * 0.5 * self.K * (-2* (self.Qinv @ self.C) + self.Qinv @ W @ self.Phi.T + self.Qinv @ W @ self.Phi)
